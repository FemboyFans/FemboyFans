    ...
    context("The forum topics controller") do
      ...
      context("show action") do
        ...
        should("have the correct page number") do
          AdminConfig.any_instance.stubs(:records_per_page).returns(2)
      ...
      context("create action") do
        should("create a new forum topic and post") do
          assert_difference({ "ForumPost.count" => 1, "ForumTopic.count" => 1, "ModAction.count" => 0 }) do
            post_auth(forum_topics_path, @user, params: { forum_topic: { title: "bababa", category_id: AdminConfig.instance.alias_and_implication_forum_category, original_post_attributes: { body: "xaxaxa" } } })
          end
        ...
        should("cause the unread indicator to show") do
          @other_user.forum_category_visits.find_or_create_by!(forum_category_id: AdminConfig.instance.alias_and_implication_forum_category).update!(last_read_at: Time.now)
        ...
        should("allow setting allow_voting=true") do
          assert_difference({ "ForumPost.count" => 1, "ForumTopic.count" => 1, "EditHistory.count" => 2 }) do
            post_auth(forum_topics_path, @user, params: { forum_topic: { title: "bababa", category_id: AdminConfig.instance.alias_and_implication_forum_category, original_post_attributes: { body: "xaxaxa", allow_voting: true } } })
            @forum_topic = ForumTopic.last
